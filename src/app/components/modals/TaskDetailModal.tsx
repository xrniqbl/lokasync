...
export function TaskDetailModal({ task, onClose, onUpdate, onDelete }: TaskDetailModalProps) {
  ...
  const handleSave = () => {
    onUpdate({ ...task, title: title.trim() || task.title, status, priority, completed: status === "completed" });
    ...
    <BaseModal open={!!task} onClose={onClose} title="" width="max-w-lg">
      <div className="-mt-5 space-y-5">
        ...
        <div>
          ...
          <div className="text-neutral-500 text-[12px] mt-1.5">{task.description}</div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 py-4 border-t border-b border-neutral-800/40">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-neutral-300 text-[13px]">{task.assignee}</span>
            </div>
          </div>
          <div>
